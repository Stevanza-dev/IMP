        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white p-4 rounded shadow border-l-4 border-red-500">
                    <p class="text-xs text-gray-500 uppercase font-bold">Tidak Hadir</p>
                    <p class="text-2xl font-bold text-red-600">{{ $totalAbsent }}</p>
                </div>
                <div class="bg-white p-4 rounded shadow border-l-4 border-purple-500">
                    <p class="text-xs text-gray-500 uppercase font-bold">Persentase</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white rounded-lg shadow overflow-hidden">
                    <div class="bg-green-50 px-6 py-4 border-b border-green-100 flex justify-between items-center">
                        <h3 class="font-bold text-green-800">✅ DAFTAR HADIR ({{ $totalPresent }})</h3>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3">
                                        ...
                                        <p class="text-xs text-gray-500">{{ $data->division }}</p>
                                    </td>
                                    <td class="px-4 py-3 font-mono text-green-700">
                                        {{ \Carbon\Carbon::parse($data->check_in_at)->format('H:i') }}
                                    </td>
                                    <td class="px-4 py-3 text-xs text-gray-500">
                                        {{ round($data->distance_in_meters) }}m
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-4 text-center text-gray-400">Belum ada yang hadir</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="bg-white rounded-lg shadow overflow-hidden">
                    <div class="bg-red-50 px-6 py-4 border-b border-red-100 flex justify-between items-center">
                        <h3 class="font-bold text-red-800">❌ TIDAK HADIR ({{ $totalAbsent }})</h3>
                    </div>
                    <div class="overflow-y-auto max-h-[500px]">
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50 sticky top-0">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 font-medium text-gray-700">{{ $member->name }}</td>
                                    <td class="px-4 py-3 text-xs text-gray-500">{{ $member->division }}</td>
                                    <td class="px-4 py-3">
                                        <span class="px-2 py-1 bg-red-100 text-red-600 rounded text-xs font-bold">Alpha</span>
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-4 text-center text-green-600 font-bold">Luar Biasa! Semua Hadir 🎉</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
