        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="GET" action="{{ route('dashboard') }}" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                        <div class="flex items-center space-x-2">
                            <select name="filter" class="rounded border-gray-200 px-3 py-2 text-sm">
                                ...
                                <option value="pending" {{ (isset($filter) && $filter === 'pending') ? 'selected' : '' }}>Belum Dikonfirmasi</option>
                            </select>
                            <button type="submit" class="bg-emerald-600 text-white px-3 py-2 rounded text-sm">Filter</button>
                        </div>

                        <div class="flex items-center space-x-2">
                            <input type="search" name="q" value="{{ $q ?? '' }}" placeholder="Cari nama..." class="rounded border-gray-200 px-3 py-2 text-sm" />
                    ...
                    <div class="md:hidden space-y-4">
                        ...
                        <div class="bg-white p-4 rounded-lg shadow-sm border">
                            ...
                            <div class="mt-3 text-sm text-gray-700">
                                ...
                                <div class="mt-1 text-xs text-gray-500 truncate" title="{{ $reg->address }}">{{ Str::limit($reg->address, 80) }}</div>
                            </div>
                                ...
                                <div class="flex items-center space-x-2">
                                    @if($reg->status == 'pending')
                                        <form action="{{ route('admin.approve', $reg->id) }}" method="POST" onsubmit="return confirm('Yakin validasi data ini?');">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="text-white bg-green-600 hover:bg-green-700 px-3 py-1 rounded text-sm" title="Terima">
                                        ...
                                        <form action="{{ route('admin.reject', $reg->id) }}" method="POST" onsubmit="return confirm('Tolak pendaftaran ini?');">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="text-white bg-yellow-500 hover:bg-yellow-600 px-3 py-1 rounded text-sm" title="Tolak">
                                                <i class="fas fa-times"></i>
                                            </button>
                                        </form>

                                        <a href="{{ route('admin.edit', $reg->id) }}" class="text-white bg-blue-500 hover:bg-blue-600 px-3 py-1 rounded text-sm" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                    @elseif($reg->status == 'confirmed')
                                        <form action="{{ route('admin.resend', $reg->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="text-blue-600 hover:text-blue-800 text-sm font-bold underline">
                                                <i class="fas fa-envelope"></i>
                                            </button>
                                        </form>
                                        <a href="{{ route('admin.edit', $reg->id) }}" class="text-gray-500 hover:text-gray-700 text-sm">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                    @else
                            ...
                            <div class="mt-3 border-t pt-2">
                                <form action="{{ route('admin.destroy', $reg->id) }}" method="POST" onsubmit="return confirm('PERINGATAN: Data akan dihapus permanen dan tidak bisa dikembalikan. Lanjutkan?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700 text-sm flex items-center gap-2">
                    ...
                    <div id="image-modal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 hidden z-50">
                        <div class="max-w-3xl w-full">
                            <div class="flex justify-end mb-2">
                                <button onclick="closeImageModal()" class="text-white bg-gray-800 bg-opacity-30 hover:bg-opacity-50 px-3 py-1 rounded">Tutup</button>
                            </div>
                            <img id="image-modal-img" src="" alt="Bukti Pembayaran" class="w-full h-auto rounded shadow-lg" />
                        </div>
                    </div>
