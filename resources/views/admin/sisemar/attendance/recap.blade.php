        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-blue-500">
                    <div class="text-gray-500 text-sm font-medium">TOTAL PESERTA (LUNAS)</div>
                    <div class="text-3xl font-bold text-gray-800">{{ $stats['total'] }}</div>
                </div>

                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-yellow-500">
                    <div class="text-yellow-600 text-sm font-bold">SUDAH TUKAR TIKET</div>
                    <div class="text-3xl font-bold text-yellow-700">{{ $stats['redeemed'] }}</div>
                </div>

                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-green-500">
                    <div class="text-green-600 text-sm font-bold">SUDAH MASUK (CHECK-IN)</div>
                    <div class="text-3xl font-bold text-green-700">{{ $stats['checked_in'] }}</div>
                    <div class="text-xs text-gray-400 mt-1">
                        {{ $stats['total'] > 0 ? round(($stats['checked_in'] / $stats['total']) * 100) : 0 }}% Kehadiran
                    </div>
                </div>
            </div>
