    ...
    <section class="min-h-screen py-12 px-4 sm:px-6 lg:px-8 mt-16 md:mt-0">
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white p-8 md:p-10 rounded-2xl shadow-xl border-t-4 border-emerald-600">
                ...
                @if (session('success'))
                    <div class="mb-6 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg flex items-start gap-3"
                        role="alert">
                ...
                @else
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email') }}" required
                            class="w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition bg-white">
                        <p class="text-xs text-gray-500 mt-2"><i class="fas fa-info-circle"></i> Tiket akan dikirim ke
                            email ini.</p>
                    ...
                    <div>
                        <label for="institution" class="block text-sm font-semibold text-gray-700 mb-2">Asal Instansi /
                            Sekolah</label>
                        <input type="text" name="institution" id="institution" value="{{ old('institution') }}" required
                            class="w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition bg-white">
                        @error('institution') <span class="text-red-500 text-xs mt-1 block"><i
                        class="fas fa-exclamation-circle"></i> {{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('address') <span class="text-red-500 text-xs mt-1 block"><i
                        class="fas fa-exclamation-circle"></i> {{ $message }}</span> @enderror
                    </div>

                    <div class="pt-4 border-t-2 border-gray-100">
                        <label for="payment_method" class="block text-sm font-semibold text-gray-700 mb-2">Metode
                            Pembayaran</label>
                        <select name="payment_method" id="payment_method" required
                            ...
                            <option value="shopeepay" {{ old('payment_method') == 'shopeepay' ? 'selected' : '' }}>🛍️
                                ShopeePay</option>
                            <option value="qris" {{ old('payment_method') == 'qris' ? 'selected' : '' }}>📱 QRIS</option>
                        </select>
                        @error('payment_method') <span class="text-red-500 text-xs mt-1 block"><i
                        class="fas fa-exclamation-circle"></i> {{ $message }}</span> @enderror
                    </div>

                    <div id="payment-info"
                        class="hidden bg-gradient-to-r from-emerald-50 to-green-50 p-6 rounded-xl border-2 border-emerald-200">
                        <div class="flex justify-between items-center">
                            <div>
                                <p class="text-xs text-gray-600 uppercase font-semibold tracking-wide">Nomor <span
                                        id="payment-label"></span></p>
                                <p class="text-xl font-bold text-gray-900 mt-2" id="payment-number"></p>
                            </div>
                            <button type="button" onclick="copyToClipboard()"
                                class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg text-sm font-semibold transition flex items-center gap-2">
                    ...
                    <div id="qris-section" class="hidden bg-emerald-50 p-6 rounded-xl border-2 border-emerald-200">
                        ...
                        <div class="flex justify-center">
                            <img src="{{ asset('images/qrisampera.jpg') }}" alt="QRIS Code"
                    ...
                    <div>
                        <label for="payment_proof" class="block text-sm font-semibold text-gray-700 mb-3">Bukti
                            Pembayaran (Screenshot/Foto)</label>
                        <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-emerald-500 hover:bg-emerald-50 transition cursor-pointer bg-gray-50"
                            id="upload-area">
                            <div class="space-y-2">
                                <div class="flex justify-center">
                                ...
                                <div class="text-sm text-gray-600">
                                    <label for="payment_proof" class="relative cursor-pointer">
                                        <span class="font-semibold text-emerald-600 hover:text-emerald-700">Klik untuk
                                            upload</span> atau drag file ke sini
                                        <input id="payment_proof" name="payment_proof" type="file" class="sr-only"
                                            accept="image/*" required onchange="handleFileSelect()">
                                    </label>
                                </div>
                        ...
                        <div id="file-selected"
                            class="hidden mt-4 p-4 bg-green-50 border-2 border-green-300 rounded-lg">
                            <p class="text-sm text-green-700"><i class="fas fa-check-circle"></i> <strong>File
                                    terpilih:</strong> <span id="file-name" class="font-semibold"></span></p>
                        </div>
                        @error('payment_proof') <span class="text-red-500 text-xs mt-2 block"><i
                        class="fas fa-exclamation-circle"></i> {{ $message }}</span> @enderror
                    </div>
