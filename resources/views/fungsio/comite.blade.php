	...
	<div class="py-6">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			...
			<div class="bg-white shadow sm:rounded-lg mb-6" id="created">
				<div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">
					<div>
				...
				<div class="block md:hidden border-b border-gray-200">
					@forelse ($comites as $comite)
						<div class="p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50">
							<div class="flex justify-between items-start mb-2">
								<div>
									<div class="text-sm font-medium text-gray-900">{{ $comite->title ?? '-' }}</div>
									<div class="text-xs text-gray-500">{{ $comite->workProgram->name ?? '-' }}</div>
								</div>
								<div>
									@switch($comite->status)
										@case('draft')
											...
											@break
										@case('pending')
											<span class="inline-flex items-center rounded-full bg-yellow-100 px-2 py-0.5 text-xs font-medium text-yellow-800">Menunggu</span>
											@break
										@case('approved')
							...
							<div class="text-xs text-gray-500 mb-3">
								Dibuat: {{ $comite->created_at?->format('d M Y, H:i') }}
							</div>
							<div class="flex items-center justify-end space-x-3 text-sm">
								<a href="{{ route('fungsio.comite.show', $comite->id) }}" class="text-indigo-600 hover:text-indigo-900 font-medium">Detail</a>
								@if (in_array($comite->status, ['draft', 'pending']))
									<a href="{{ route('fungsio.comite.edit', $comite->id) }}" class="text-yellow-600 hover:text-yellow-900 font-medium">Edit</a>
					...
					@empty
						<div class="p-4 text-center text-sm text-gray-500">Belum ada kepanitiaan yang Anda buat.</div>
					@endforelse
				</div>

				{{-- Desktop Table View --}}
				<div class="hidden md:block px-6 py-4 overflow-x-auto">
			...
			<div class="bg-white shadow sm:rounded-lg" id="my">
				...
				<div class="block md:hidden border-b border-gray-200">
						...
						<div class="p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50">
							<div class="flex justify-between items-start mb-2">
								<div>
									...
									<div class="text-xs text-gray-500">{{ $row['program'] }}</div>
								</div>
								<div>
									@switch($row['status'])
										@case('pending')
							...
							<div class="mb-2 space-y-1">
								<div class="text-xs text-gray-500">
									<span class="font-medium text-gray-700">Sie:</span> {{ $row['sie'] }}
								</div>
								<div class="text-xs text-gray-500 flex items-center gap-1">
									<span class="font-medium text-gray-700">Peran:</span>
									@if ($row['role'] === 'ketua')
										<span class="inline-flex items-center rounded-full bg-purple-100 px-1.5 py-0.5 text-[10px] font-medium text-purple-800">Ketua Panitia</span>
									@elseif ($row['role'] === 'koor')
										<span class="inline-flex items-center rounded-full bg-indigo-100 px-1.5 py-0.5 text-[10px] font-medium text-indigo-800">Koordinator</span>
									@else
										<span class="inline-flex items-center rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-800">Anggota</span>
									@endif
								</div>
							</div>
							<div class="text-xs text-gray-500 mb-3 block">
								Dibuat: {{ optional($row['created_at'])->format('d M Y, H:i') }}
							</div>
					...
					@empty
						<div class="p-4 text-center text-sm text-gray-500">Anda belum terdaftar dalam kepanitiaan mana pun.</div>
					@endforelse
				</div>
