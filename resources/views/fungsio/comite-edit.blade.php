        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-medium text-gray-900">Form Edit Kepanitiaan</h3>
                        <p class="mt-1 text-sm text-gray-500">Ubah informasi dasar dan daftar sie kepanitiaan.</p>
                    </div>
                    <a href="{{ route('fungsio.comite.index') }}" class="text-sm text-indigo-600 hover:text-indigo-500">&larr; Kembali</a>
                </div>

                <div class="px-6 py-4">
                    <form method="POST" action="{{ route('fungsio.comite.update', $comite->id) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="work_program_id" name="work_program_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                @foreach ($workPrograms as $program)
                                    <option value="{{ $program->id }}" @selected(old('work_program_id', $comite->work_program_id) == $program->id)>
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" value="{{ old('title', $comite->title) }}" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $comite->description) }}</textarea>
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label value="Daftar Sie" />
                            <p class="mt-1 text-xs text-gray-500">Ubah atau tambahkan nama sie sesuai kebutuhan. <span class="text-amber-600 font-medium">Perhatian: Menghapus sie akan menghapus semua anggota di sie tersebut.</span></p>

                            @php
                                $oldSies = old('sies');
                                $sieNames = $oldSies !== null
                                    ? $oldSies
                                    : $comite->sies->pluck('name')->toArray();
                            @endphp

                            <div id="sie-container" class="mt-3 space-y-2">
                                @forelse ($sieNames as $name)
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        container.addEventListener('click', function (e) {
            ...
            if (removeButton) {
                ...
                if (confirm('Menghapus sie akan menghapus semua anggota di sie tersebut. Lanjutkan?')) {
                    sieItem.remove();
                }
            }
        });
